...
const verifyEmail = (req: Request, res: Response, next: NextFunction) => {
    ...
    models.User.findOne({ where: { email: req.body.email } })
        .then((result: any) => result ? res.status(409).json({ message: "Email already registered!" }) : singUp(req, res, next))
        .catch((err: any) => {
...
const loginUser = (req: Request, res: Response, next: NextFunction) => {
    ...
    models.User.findOne({ where: { email: req.body.email } })
        .then((user: any) => {
            user === null ? res.status(401).json({
                message: "Email not registered"
            })
                :
                bcryptjs.compare(req.body.password, user.password, (err: any, result: any) => {
                    if (result) {
